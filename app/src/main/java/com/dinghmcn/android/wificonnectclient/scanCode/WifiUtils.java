...
public final class WifiUtils {
     ...
     * @param wType 加密类型
     */
    public WifiConfiguration quickConnWifi(String ssid, String pwd, int wType) {
     ...
     */
    public WifiConfiguration quickConnWifi(String ssid, String pwd, int wType, boolean isStatic, String ip) {
        // 步骤
        // 1.创建Wifi静态Ip连接配置
        // 2.创建正常Wifi连接配置
        ...
        try {
            ...
            if (isStatic && !TextUtils.isEmpty(ip)) {
                ...
                if (staticWifiConfig == null) {
                    ...
                    // --
                    Log.e(TAG, "属于正常方式连接(DHCP)");
                } else {
                    // 设置静态信息
                    connWifiConfig = staticWifiConfig;
                    // --
                    Log.e(TAG, "属于静态IP方式连接");
                }
            } else {
                // 创建正常的配置信息
                connWifiConfig = createWifiConfig(ssid, pwd, wType, true);
            ...
            if (preWifiConfig != null) {
                ...
                // 打印结果
                Log.e(TAG, "删除旧的配置信息 - " + preWifiConfig.SSID + ", isRemove: " + isRemove);
                // 保存配置
                mWifiManager.saveConfiguration();
            }
            // --
            ...
            if (_nId != -1) {
                try {
                    ...
                    // 打印断开连接结果
                    Log.e(TAG, "isDisConnect : " + isDisConnect + ", isDisable : " + isDisable);
                } catch (Exception e) {
                    Log.e(TAG, "quickConnWifi - 关闭连接出错：" + _nId);
                }
                // 开始连接
                ...
                // 打印结果
                Log.e(TAG, "addNetwork(enableNetwork) - result : " + isResult);
            } else {
                // 尝试不带引号SSID连接
            ...
            // 连接的networkId
            Log.e(TAG, "连接的SSID - networkId : " + _nId);
            // 返回连接的信息
            return connWifiConfig;
        } catch (Exception e) {
            Log.e(TAG, "quickConnWifi", e);
        }
        return null;
    }
     ...
     */
    public static WifiConfiguration createWifiConfig(String ssid, String pwd, int wType, boolean isHandler) {
        try {
            // 创建一个新的网络配置
